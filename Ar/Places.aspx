<%@ Page Title="" Language="C#" MasterPageFile="~/Ar/MasterPages/MasterPage.master" AutoEventWireup="true" CodeFile="Places.aspx.cs" Inherits="Ar_Places" %>
...
.categories-bar-wrapper {
    ...
    margin-bottom: 20px; /* مسافة عن باقي المحتوى */
}
...
.categories-bar-scroller {
    ...
    -webkit-overflow-scrolling: touch; /* تحسين التمرير على الهواتف */
    gap: 20px; /* مسافة بين العناصر */
    padding: 0 10px; /* لإعطاء مساحة عند التمرير */
}
...
.category-item {
    ...
    color: #333;
    min-width: 65px; /* تثبيت العرض لكي لا تنكمش */
    flex-shrink: 0; /* منع العناصر من الانكماش */
    transition: transform 0.2s;
}

.category-item:hover {
    color: #ffc119; /* لون مميز عند المرور */
}

/* تنسيق الصورة */
.category-item img {
    width: 50px; /* حجم الأيقونة/الصورة */
    height: 50px;
...
.category-item.active {
    color: #ffc119; /* هذا السطر يغير لون الكتابة (النص) */
}
...
.categories-list-scroll {
    ...
    gap: 10px; /* المسافة بين التصنيفات */
    padding: 0 15px; /* مسافة على الجانبين */
}
...
.category-pill {
    ...
    align-items: center;
    padding: 8px 15px;
    border-radius: 20px; /* شكل الحبة (Pill Shape) */
    background-color: #fff;
    color: #333;
    text-decoration: none;
    font-size: 14px;
    white-space: nowrap; /* مهم: منع النص من النزول لسطر جديد */
    border: 1px solid #eee;
    transition: background-color 0.2s, color 0.2s, border-color 0.2s;
    flex-shrink: 0; /* مهم: منع العناصر من الانكماش */
}

.category-pill img {
    width: 20px;
    height: 20px;
    margin-inline-end: 8px; /* مسافة بين الصورة والنص */
}

/* 4. حالة العنصر النشط */
.category-pill.active {
    background-color: #ffc119; /* اللون المميز */
    color: #fff;
    border-color: #ffc119;
}
...
@media (max-width: 768px) {
    .food-categories-mobile-bar {
        display: block; /* إظهار الشريط في الشاشات الصغيرة */
    }
    ...
    <span class="route">
    <a href="Default.aspx"><asp:Literal ID="ltHome" runat="server" Text="<%$ Resources:texts, Home %>"></asp:Literal></a>
    ...
    <div class="selectedLocationFilters">
      <figure class="locationShops">
        <div class="inputHolder">
          ...
          <input type="text" name="selectedShopSearcher" id="selectedShopSearcher" placeholder=<%= Resources.Texts.Search %>>
        </div>
        <article class="mainFilters">
          <span><asp:Literal ID="ltSortBy" runat="server" Text="<%$ Resources:texts, SortBy %>"></asp:Literal> </span>
    ...
    <span class="filterCategory" id="deliveryFeeOrder">
        <asp:Literal ID="ltSortDeliveryFee" runat="server" Text="<%$ Resources:texts, Sort_DeliveryFee %>"></asp:Literal>
    </span>
    ...
    <span class="filterCategory" id="minPayOrder">
        <asp:Literal ID="ltSortMinOrder" runat="server" Text="<%$ Resources:texts, Sort_MinOrder %>"></asp:Literal>
    </span>
...
function formatAddress(state) {
    ...
    white-space: normal;  /* يسمح بالنص المتعدد الأسطر */
    line-height: 1.2em;   /* ارتفاع كل سطر */
    padding-top: 0.3rem;  /* مسافة علوية */
    padding-bottom: 0.3rem; /* مسافة سفلية */
    display: block;
